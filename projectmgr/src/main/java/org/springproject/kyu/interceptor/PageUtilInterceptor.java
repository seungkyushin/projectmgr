...
public class PageUtilInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if (httpSession != null && httpSession.getAttribute("email") == null) {
			...
			if (cookies != null) {
				int cookieMax = cookies.length;
				for (int i = 0; i < cookieMax; i++) {
	...
	public void makeCookie(String attributeName, HttpServletRequest req, HttpServletResponse res) {
	...
	public Cookie createCookieMsg(String attributeName, String msg) {
		try {
			...
			if (msg.matches(".*[ㄱ-ㅎㅏ-ㅣ가-힣]+.*")) {
				resultMsg = new Cookie(attributeName, URLEncoder.encode(msg, "utf-8"));
			} else {
